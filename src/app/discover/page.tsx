...
const categories = [
  ...
  { id: 'food-walk', label: 'Culinary Trails' },
  { id: 'heritage', label: 'Heritage' },
  { id: 'cultural', label: 'Cultural' },
  { id: 'nature', label: 'Nature & Spice' },
  { id: 'wellness', label: 'Wellness & Yoga' },
  { id: 'workshop', label: 'Art & Craft' },
  { id: 'water-sports', label: 'Ocean & Dive' },
];
      ...
      <nav className="sticky top-0 z-50 glass-card border-b border-[var(--border)] rounded-none px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/" className="flex items-center gap-2.5 hover:opacity-85 transition-opacity">
            <div className="w-8 h-8 rounded-lg bg-slate-900 dark:bg-blue-600 flex items-center justify-center shadow-md">
        ...
        <div className="mb-6 space-y-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 rounded-2xl bg-[var(--surface)] border border-[var(--border)] text-sm focus:outline-none focus:border-blue-500 transition-colors shadow-xs"
            />
          </div>

          {/* Category pills - Lucide Icons, Zero Emojis, No Rainbows */}
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
            {categories.map((cat) => (
              ...
              >
                <NodeIcon typeOrIcon={cat.id} className="w-3.5 h-3.5" />
                {cat.label}
              </button>
            ))}
          </div>
              ...
              <div className="h-64 sm:h-72 w-full relative overflow-hidden bg-slate-900">
                {selectedExperience.images && selectedExperience.images[0] ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-slate-500 bg-slate-900">
                    <NodeIcon typeOrIcon={selectedExperience.category} className="w-16 h-16" />
                  </div>
                )}
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                  {selectedExperience.isHidden && (
                    <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-slate-900/90 text-white backdrop-blur-md shadow-sm border border-slate-700">
                      <Sparkles className="w-3.5 h-3.5 text-blue-400" /> Hidden Gem
                    </span>
                  )}
      ...
      <div className="h-48 bg-slate-900 relative overflow-hidden">
        {exp.images && exp.images.length > 0 ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-slate-800">
            <NodeIcon typeOrIcon={exp.category} className="w-12 h-12 text-slate-400" />
          </div>
        )}
        ...
        <div className="absolute top-3 left-3 flex gap-1.5">
          {exp.isHidden && (
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-purple-600/90 text-white backdrop-blur-md shadow-xs flex items-center gap-1">
